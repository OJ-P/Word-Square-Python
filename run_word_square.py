...
def word_square():  # func to take user input and output a word square
    def get_user_input():  # asks user for user input and formats it
        ...
        def validate_user_input(u_input, length):  # validates if user input is usable
            if not u_input[0].isdigit():  # checks if first character of string is a number
                print("\nInvalid input - the first character must be a number\n")
                return False
            elif int(u_input[0]) == 0:  # checks if first character of string is greater than zero
                print("\nInvalid input - the first character can not be zero\n")
                return False
            elif (int(u_input[0]) ** 2) != length - 1:  # int value at index[0] of user_input squared must equal str_length
                print("\nInvalid input - input length must equal first character (number) squared\n")
                return False
            for current_char in u_input[1:length:1]:  # [start:end:step] iterates through input string from index[1]
                if not current_char.isalpha():  # evaluates if current char is alphabetical
                    ...
                    return False
                elif current_char == u_input[length - 1]:  # if all chars are alphabetical, returns valid input
                    return True
    ...
    def create_word_squares(word_list, word_length):
        ...
        def backtracking(index, current_word_list, word_size):
            ...
            prefix = ''.join(word[index] for word in current_word_list)  # sets prefix to the letter at the index of the current word in the list i.e. word:"aced" index:1 prefix = "c"
            for candidate in get_prefixes(prefix):  # for candidate (the current word e.g. w1) in prefix dict list: prefixes:{'c':[w1, w2, w3, w4]} - statement essentially "for candidate in prefixes[prefix]"
                current_word_list.append(candidate)  # add the candidate to the current word square in the making (current_word_list)
                backtracking(index + 1, current_word_list,word_size)  # call backtracking to repeat process to add another word to the current square
